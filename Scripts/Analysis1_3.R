...
## This will perform the same analysis as Analysis1_1.R except that it is limited to early stage cancers only
##-----------------------------------------------------------------------------------------------------------
...
main <- function(Data_Names, category){
  ...
  for (i in 1:length(Data_Names)){
    ...
    for(j in 1:length(pipelines)){
      ...
      Set_1 <- df %>% filter(status == cats[1], pipeline == pipelines[j], tumor_stage == "stage i" | tumor_stage == "stage ia" | tumor_stage == "stage ib")
      Set_2 <- df %>% filter(status == cats[2] , pipeline == pipelines[j], tumor_stage == "stage i" | tumor_stage == "stage ia" | tumor_stage == "stage ib")
      ...
      tryCatch({
        ...
        write_delim(Overview, file.path(paste0("~/Research/BiolinksAnalysis/Output/",category, "/pValues/total_mut_pval/", pipelines[j], "/", "Overall_Stage_1", 
                                               "_", Data_Names[i],
                                               ".tsv")), delim  = "\t")
      }, error = function(e){})
      ggsave(total_mut, file = file.path(paste0("~/Research/BiolinksAnalysis/Output/",category, "/pValues/total_mut_pval/", pipelines[j], "/", "Overall_Stage_1", 
                                                "_",
                                                Data_Names[i], 
                                                ".jpg")), width = 6,
             height = 6, units = "in")
      ...
      ggsave(nuc_graph, file = file.path(paste0("~/Research/BiolinksAnalysis/Output/",category,"/Graphs/nucChange_Graph/", pipelines[j], "/","Overall_Stage_1", 
                                                "_", 
                                                Data_Names[i],
                                                ".jpg")), width = 6,
             height = 6, units = "in")
      for(n in 1:length(nucChanges)){
        ...
        tryCatch({
          ...
          write_delim(change_summary, file.path(paste0("~/Research/BiolinksAnalysis/Output/",category,"/pValues/nucChange_pVal/",pipelines[j], "/", "Overall_Stage_1", 
                                                       "_",
                                                       Data_Names[i],
                                                       "_", nucChanges[n],
                                                       ".tsv")), delim  = "\t")
      ...
      for(l in 1:length(type)){
        ...
        tryCatch({
          ...
          write_delim(change_summary, file.path(paste0("~/Research/BiolinksAnalysis/Output/",category, "/pValues/TiTv_pVal/", pipelines[j], "/", "Overall_Stage_1", 
                                                       "_",
                                                       Data_Names[i],
                                                       "_", type[l],
                                                       ".tsv")), delim  = "\t")
