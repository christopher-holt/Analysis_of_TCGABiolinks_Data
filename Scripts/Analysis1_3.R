...
for (i in 1:length(Data_Names)){
  ...
  for(j in 1:length(pipelines)){
    ...
    Set_1 <- df %>% filter(is.na(cigarettes_per_day), pipeline == pipelines[j], tumor_stage == "stage i" | tumor_stage == "stage ia" | tumor_stage == "stage ib") 
    Set_2 <- df %>% filter(!(is.na(cigarettes_per_day)), pipeline == pipelines[j], tumor_stage == "stage i" | tumor_stage == "stage ia" | tumor_stage == "stage ib")
    ...
    tryCatch({
      ...
      write_delim(Overview, file.path("~/Research/BiolinksAnalysis/Output/Smoke/pValues/total_mut_pval/", paste0(pipelines[j], "/", "Overall_Stage_1", 
                                                                                                                 "_", Data_Names[i],
                                                                                                                 ".tsv")), delim  = "\t")
    }, error = function(e){})
    ggsave(total_mut, file = file.path("~/Research/BiolinksAnalysis/Output/Smoke/Graphs/total_mut_Graph/", paste0(pipelines[j], "/", "Overall_Stage_1", 
                                                                                                                  "_",
                                                                                                                  Data_Names[i], 
                                                                                                                  ".jpg")), width = 6,
           height = 6, units = "in")
    ...
    ggsave(nuc_graph, file = file.path("~/Research/BiolinksAnalysis/Output/Smoke/Graphs/nucChange_Graph/", paste0(pipelines[j], "/","Overall_Stage_1", 
                                                                                                                  "_", 
                                                                                                                  Data_Names[i],
                                                                                                                  ".jpg")), width = 6,
           height = 6, units = "in")
    for(n in 1:length(nucChanges)){
      ...
      tryCatch({
        ...
        write_delim(change_summary, file.path("~/Research/BiolinksAnalysis/Output/Smoke/pValues/nucChange_pVal/", paste0(pipelines[j], "/", "Overall_Stage_1", 
                                                                                                                         "_",
                                                                                                                         Data_Names[i],
                                                                                                                         "_", nucChanges[n],
                                                                                                                         ".tsv")), delim  = "\t")
    ...
    ggsave(TiTv, file = file.path("~/Research/BiolinksAnalysis/Output/Smoke/Graphs/TiTv_Graph/", paste0(pipelines[j], "/", "Overall_Stage_1", 
                                                                                                        "_",
                                                                                                        Data_Names[i], 
                                                                                                        ".jpg")), width = 6,
           height = 6, units = "in")
    ...
    for(l in 1:length(type)){
      ...
      tryCatch({
        ...
        write_delim(change_summary, file.path("~/Research/BiolinksAnalysis/Output/Smoke/pValues/TiTv_pVal/", paste0(pipelines[j], "/", "Overall_Stage_1", 
                                                                                                                    "_",
                                                                                                                    Data_Names[i],
                                                                                                                    "_", type[l],
                                                                                                                    ".tsv")), delim  = "\t")
