...
for (i in 1:length(Data_Names)){
  ...
  for(j in 1:length(pipelines)){
    ...
    Set_1 <- df %>% filter(gender == "male",  pipeline == pipelines[j]) %>% select(age_at_diagnosis, hugo_symbol)
    Set_2 <- df %>% filter(gender == "female",  pipeline == pipelines[j]) %>% select(age_at_diagnosis, hugo_symbol)
    ...
    write_delim(dat, file.path("Output/Gender/Age/files/", paste0(pipelines[j], "/", Data_Names[i], "_rsquared.tsv")),
                delim = '\t')
    
    ggsave(age_graph, file = file.path("Output/Gender/Age/Graphs/", paste0(pipelines[j], "/", Data_Names[i], "_age_burden.jpg")),
