...
for (i in 1:length(Data_Names)){
  ...
  for(j in 1:length(pipelines)){
    ...
    Set_1 <- df %>% filter(is.na(cigarettes_per_day), pipeline == pipelines[j]) %>% select(age_at_diagnosis, hugo_symbol)
    Set_2 <- df %>% filter(!(is.na(cigarettes_per_day)), pipeline == pipelines[j]) %>% select(age_at_diagnosis, hugo_symbol)
    ...
    ggsave(age_graph, file = file.path("Output/Smoke/Age/Graphs/", paste0(pipelines[j], "/", Data_Names[i], "_age_burden.jpg")),
