...
for (i in 1:length(cancers)){
  temp <- main_df_som %>% filter(cohort == cancers[i], sig == 1, pipeline == "somaticsniper")
  if(nrow(temp) > 0){
    assign(cancers[i], temp)
  }
}
genes <- unique(main_df_som$gene) ## genes that are significant
sig_genes = c()
...
ggplot(main_df_som) + geom_tile(aes(cohort, gene, fill = sig)) + theme(axis.text.y = element_blank())
