...
main <- function(Data_Names, category){
  ...
  for(j in 1:length(pipelines)){
    ...
    valid_sites <- sites(df_1, category)
    ...
    ## Set_1 are non-smokers, Set_2 are smokers
        Set_1 <- df %>% filter(status == cats[1], pipeline == pipelines[j], tissue_or_organ_of_origin == valid_sites[k])
        Set_2 <- df %>% filter(status == cats[2] , pipeline == pipelines[j], tissue_or_organ_of_origin == valid_sites[k])
        ...
        tryCatch({
          ...
          write_delim(Overview, file.path(paste0("~/Research/BiolinksAnalysis/Output/",category, "/pValues/total_mut_pval/", pipelines[j], "/", 
                                                 Data_Names[i], "_", valid_sites[k],
                                                 ".tsv")), delim  = "\t")
        }, error = function(e){})
        ggsave(total_mut, file = file.path(paste0("~/Research/BiolinksAnalysis/Output/",category, "/pValues/total_mut_pval/", pipelines[j], "/", 
                                                  Data_Names[i], "_", valid_sites[k],
                                                  ".jpg")), width = 6,
               height = 6, units = "in")
        ...
        ggsave(nuc_graph, file = file.path(paste0("~/Research/BiolinksAnalysis/Output/",category,"/Graphs/nucChange_Graph/", pipelines[j], "/", 
                                                  Data_Names[i], "_", valid_sites[k],
                                                  ".jpg")), width = 6,
               height = 6, units = "in")
        for(n in 1:length(nucChanges)){
          ...
          tryCatch({
            ...
            write_delim(change_summary, file.path(paste0("~/Research/BiolinksAnalysis/Output/",category,"/pValues/nucChange_pVal/",pipelines[j], "/", 
                                                         Data_Names[i], "_", valid_sites[k],
                                                         "_", nucChanges[n],
                                                         ".tsv")), delim  = "\t")
        ...
        ggsave(TiTv, file = 
                 file.path(paste0("~/Research/BiolinksAnalysis/Output/", category, "/Graphs/TiTv_Graph/",
                                  pipelines[j], "/", 
                                  Data_Names[i], "_", valid_sites[k], ".jpg")), width = 6, height = 6, units = "in")
        ...
        for(l in 1:length(type)){
          ...
          tryCatch({
            ...
            write_delim(change_summary, file.path(paste0("~/Research/BiolinksAnalysis/Output/",category, "/pValues/TiTv_pVal/", pipelines[j], "/", 
                                                         Data_Names[i], "_", valid_sites[k],
                                                         "_", type[l],
                                                         ".tsv")), delim  = "\t")
          }, error = function(e){})
